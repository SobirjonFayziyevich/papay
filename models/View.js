...
class View {
  ...
  async validateChosenTarget(view_ref_id, group_type) {
    try {
      ...
      let result;
      switch (
        group_type //switch argumenti group_type orqali kerakli kollekshinlardan izlaymiz.
      ) {
        case "member": // faqat memberlarni tomosha qiyayotganimz un member quyamiz.
          result = await this.memberModel //memberSchema modelni chaqirayopmiz.
            .findOne({
              // findOne mathodi ni urniga agarda findById mathodi bulsa, keyingi bosqichga xatolikni kursatmasdan utkazib yuboraveradi.
              _id: view_ref_id,
              mb_status: "ACTIVE",
            })
            .exec();
