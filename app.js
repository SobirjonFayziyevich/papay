...
const store = new MongoDBStore({
  // MongoDBStore orqali (store) objectini yasadik
...
app.use(
  cors({
    //faqat URL dan keladigan malumotlarni qabul qilsin degani.
    credentials: true,
...
const io = require("socket.io")(server, {
  // buyerda qushimcha shartlar kiritib oldim.socket.io docuentatinda yuq mantiq.
  serveClient: false,
...
io.on("connection", function (socket) {
  // kimdir bizni serverga ulansa xabar shu yerga keladi degani.
  ...
  socket.on("disconnect", function () {
    // ulangan userimiz boglanolmaganda,
  ...
  socket.on("createMsg", function (data) {
    // biror bir user xabar yozsin, yani createMsg kirib kelgan vaqtda
